...
class Auth extends \fast\Auth
{
     ...
     * @return array
     */
    public function getCampusId($cId){
       $campusId=array($cId);
       if($cId=='*'){
         $list=Db::name('class_campus')->field('id,store_name')->select();
         $campusId=array_column($list,'id');
         $storeName=array_column($list,'store_name','id');
         $data=array('campus_id'=>$campusId,'store_name'=>$storeName);
       }
       if(strlen($cId)>1){
         $campusId=explode(',',$cId);
         $list=Db::name('class_campus')->field('store_name')->where('id','IN',$campusId)->select();
         $storeName=array_combine($campusId,array_column($list,'store_name'));
         $data=array('campus_id'=>$campusId,'store_name'=>$storeName);
       }
       if(empty($data)){
        $storeName=Db::name('class_campus')->field('store_name')->where('id',$cId)->find();
        $data=array('campus_id'=>$cId,'store_name'=>$storeName);
       }
       return $data;
    }
