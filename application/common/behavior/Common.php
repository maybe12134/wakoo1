...
class Common
{
    ...
    public function run(&$params){
        ...
        $user=$visitor->where(['class_user_id' => $params['id'],'create_time' => $time])->find();
        if(empty($user)){
            $visitor->data([
                'class_user_id' =>$params['id'],
                'create_time'   =>$time
            ]);
            $result=$visitor->allowField(true)->save();
        }
    }


    public function indent(&$data){
       $time=date("Y-m-d");
       $data['create_time'] = $time;
       $indent=model('ClassIndent');
       $indent->data($data);
       $result=$indent->allowField(true)->save();
       if($result===false){
          $this->error('订单表写入失败');
       }
    }
